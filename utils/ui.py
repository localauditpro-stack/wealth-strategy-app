...
def parse_currency_input(label, default_value, help_text=None, key=None):
    ...
    """
    # Convert default relative to key if needed? No, just render
    if key and key in st.session_state:
        # If exists in state, use that, but we probably want 'value' to be the default if not set?
        # Streamlit handles key persistence automatically for 'value' usually IF not set?
        # Actually easier to just use 'value' as the initial if key not populated
        pass
